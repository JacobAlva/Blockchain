...
PRIVATE_KEY = '15293600873723366065666355711114685695923056229265227436599182310456056097761'
MINER_ADDRESS = '1K9FDCn3hBnZl67OhwrOOCrfbcgob3XiYP'
SIGNHASH_ALL = 1
...
class Tx:
    ...
    """To create a transaction ID, concatenate all variables in the __init__(...) fxn and run the hash256 on it
        # First convert the variables to bytes
        # Create a serialzed fxn for the tx object and class.
    """
    def serialize(self):
    ...
    def is_coinbase(self):
        """
         ...
         # Grab the firt inpute and check if the prev_tx is b'\x00' * 32 i.e.. 32 zeros in bytes
    ...
    def to_dict(self):
        ...
        for tx_index, tx_in in enumerate(self.tx_ins):
            if self.is_coinbase():
                tx_in.script_sig.cmds[0] = little_endian_to_int(tx_in.script_sig.cmd[0]) #convert blockheight from little endian to integer

            tx_in.prev_tx = tx_in.prev_tx.hex()

            for index, cmd in enumerate(tx_in.script_sig.cmds):
                if isinstance(cmd, bytes):
                    tx_in.script_sig.cmds[index] = cmd.hex()         #convert the bytes into hexadecimal
        ...
        if self.is_coinbase():
            ...
            self.tx_ins[0].script_sig = self.tx_ins[0].script.sig.__dict__

        self.tx_ins[0] = self.tx_ins.__dict__"""

        """
        Convert Transaction Output to dict
         # If there are numbers, we don't need to do anything
         # If values are in bytes, convert it to hex
