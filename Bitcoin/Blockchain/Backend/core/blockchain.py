...
from Blockchain.Backend.util.util import merkle_root, target_to_bits
...
class Blockchain:
    ...
    """Keep track of all the unspend transcation in cache money for fast retrieval"""
    ...
    def calculate_fee(self):
        ...
        for TxId_index in self.remove_spent_Transactions:
            if TxId_index[0].hex() in self.utxos:
                self.input_amout += self.utxos[TxId_index[0].hex()].tx_outs[TxId_index[1]].amount
        ...
        self.fee = self.input_amout - self.output_amount
        
    def addBlock(self, BlockHeight, prevBlockHash):
        self.read_transaction_from_memorypool()
        timestamp = int(time.time())
        #Transaction = f"Jacob sent {BlockHeight} Bitcoins to Joe"
